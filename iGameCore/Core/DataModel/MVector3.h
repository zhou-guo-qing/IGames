...
public:
    ...
    MVector3 cross(const MVector3& b) const {
        return MVector3(y() * b.z() - b.y() * z(), -(x() * b.z() - b.x() * z()),
                        x() * b.y() - b.x() * y());
    }
...
inline double signedAngle(const MVector3& a, const MVector3& b,
                          const MVector3& n) {
    double cosTheta = dot(a, b);
...
inline void buildOrthoBasis_naive(MVector3& dir, MVector3& dir1,
                                  MVector3& dir2) {
    dir.normalize();
    if (dir[1] != 0.0 && dir[2] != 0.0) {
        ...
        dir2 = MVector3(1.0, dir[1] / dir[0],
                        -(dir[1] * dir[1] + dir[0] * dir[0]) /
                                (dir[0] * dir[2]));
    } else if (dir[0] != 0.0 && dir[1] != 0.0) {
        dir1 = MVector3(0.0, -dir[2] / dir[1], 1.0);
        dir2 = MVector3(-(dir[1] * dir[1] + dir[2] * dir[2]) /
                                (dir[0] * dir[1]),
                        1.0, dir[2] / dir[1]);
    } else if (dir[0] == 0.0 && dir[1] == 0.0) {
...
inline void buildOrthoBasis(MVector3& normal, MVector3& tangent,
                            MVector3& binormal) {
    // pick any Unit-Vector that's not parallel to normal:
    normal.normalize();
    if (fabs(normal[0]) > fabs(normal[1])) tangent = MVector3(0.0, 1.0, 0.0);
    else
