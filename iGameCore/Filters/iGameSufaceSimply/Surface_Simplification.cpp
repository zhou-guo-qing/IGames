...
//    MVector3 p_vh = vh->position(); //µãµÄÎ»ÖÃ
...
//            a = face_nor[0], b = face_nor[1], c = face_nor[2],
//            d = -dot(face_nor, p_vh);
//            p = {a, b, c, d};
...
//                double a = face_nor[0], b = face_nor[1], c = face_nor[2],
//                       d = -dot(face_nor, p_vh);
...
//                MPolyFace *face_hh = hh_boundary->pair()->polygon(),
//                          *face_prev_hh = prev_hh_boundary->pair()->polygon();
//                MVector3 face_hh_nor = face_hh->normal(),
//                         face_prev_hh_nor = face_prev_hh->normal();
//                MVector3 vir_face_hh_nor =
//                                 cross(hh_boundary->tangent(), face_hh_nor)
//                                         .normalized(),
//                         vir_face_prev_hh_nor =
//                                 cross(prev_hh_boundary->tangent(),
//                                       face_prev_hh_nor)
//                                         .normalized();
//                double a = vir_face_hh_nor[0], b = vir_face_hh_nor[1],
//                       c = vir_face_hh_nor[2], d = -dot(vir_face_hh_nor, p_vh);
//                Eigen::Matrix<double, 4, 1> p = {a, b, c, d};
//                Q_temp += lambda * (p * p.transpose());
//                a = vir_face_prev_hh_nor[0], b = vir_face_prev_hh_nor[1],
//                c = vir_face_prev_hh_nor[2],
//                d = -dot(vir_face_prev_hh_nor, p_vh);
//                p = {a, b, c, d};
//                Q_temp += lambda * (p * p.transpose());
//            }
//        }
...
//        vh = v_from;
//    }
//    if (is_collapse) { update_Q_and_Cost(vh, mesh); }
//    return is_collapse;
//}
//
//void QEM(PolyMesh* mesh) {
//    if (mesh->numVertices() == 3) return;
//    /// Initial Qv
...
//    int target_num = std::min((int)(0.2*N_V), 1000);
...
//            if (eh->index() != -1) {
//                if (QEM_collapse(temp_edge, mesh)) { N_V--; }
//            }
//        }
//    }
//}
...
//}
//bool SurFaceSimply::Execute()
//{ 
//    //std::cout << " SurFaceSimply start \n";
//    //UpdateProgress(0);
