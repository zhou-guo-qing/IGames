...
igQtFramelessWidget::igQtFramelessWidget(QObject* parent) : QObject(parent)
{
	padding = 0;
	moveEnable = true;
	resizeEnable = true;
...
bool igQtFramelessWidget::eventFilter(QObject* watched, QEvent* event)
{
	if (widget != 0 && watched == widget) {
		if (event->type() == QEvent::Resize) {
			...
			if (moveEnable) {
				if (pressed) {
					auto x = widget->x() + offsetX;
					auto y = widget->y() + offsetY;
					x = qBound(limitX1, x, limitX2 - widget->width());
					y = qBound(limitY1, y, limitY2 - widget->height());
					widget->move(x, y);
				}
			}
			...
			if (resizeEnable) {
				if (pressedLeft) {
					...
					//offsetX = std::min(offsetX, limitX2 - rectX - rectW);
...
void igQtFramelessWidget::setLimitPosition(int x1, int y1, int x2, int y2)
{
	this->limitX1 = x1;
	this->limitX2 = x2;
	this->limitY1 = y1;
...
void igQtFramelessWidget::setPadding(int padding)
{
	this->padding = padding;
}

void igQtFramelessWidget::setMoveEnable(bool moveEnable)
{
	this->moveEnable = moveEnable;
}

void igQtFramelessWidget::setResizeEnable(bool resizeEnable)
{
	this->resizeEnable = resizeEnable;
}

void igQtFramelessWidget::setWidget(QWidget* widget)
{
	if (this->widget == 0) {
		this->widget = widget;
		//设置鼠标追踪为真
