...
igQtBasicColorAreaWidget::igQtBasicColorAreaWidget(QWidget* parent) : QWidget(parent) {
	// 初始化网格布局
	LayoutGrid = new QGridLayout(this);
	LayoutGrid->setAlignment(Qt::AlignCenter | Qt::AlignTop);
	...
	foreach(color, colorList) {
		strList = color.split(",");
		igQtBasicColorItem* BasicColorItem = new igQtBasicColorItem(QColor(strList.at(0).toInt(), strList.at(1).toInt(), strList.at(2).toInt()));
		connect(BasicColorItem, SIGNAL(sigItemClicked(const QColor&)), this, SIGNAL(sigColorItemSel(const QColor&)));
		LayoutGrid->addWidget(BasicColorItem, LayoutGrid->count() / 8, LayoutGrid->count() % 8);
	}
}
igQtBasicColorAreaWidget::~igQtBasicColorAreaWidget() {
}
